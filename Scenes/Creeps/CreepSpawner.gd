...
func _ready():
	...
	Log.debug("Creep scenes have been loaded.")
...
func queue_spawn_creep(creep: Creep):
	...
	if _timer_between_creeps.is_stopped():
		...
		Log.debug("Start creep spawn timer with delay [%s]." % _timer_between_creeps.get_wait_time())
...
func spawn_creep(creep: Creep):
	if not creep:
		Log.debug("Stop creep spawn. Queue is exhausted.")
	...
	Log.debug("Creep has been spawned [%s]." % creep)
