...
func tower_init():
	...
	sentry_bt.set_buff_icon("res://Resources/Textures/GenericIcons/semi_closed_eye.tres")
...
func ball_pt_on_hit(projectile: Projectile, creep: Unit):
	...
	if exploded:
		var effect: int = Effect.create_scaled("FireLordDeathExplode.mdl", Vector3(projectile.get_x(), projectile.get_y(), 30.0), 0, 5)
