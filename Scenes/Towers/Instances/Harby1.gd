...
#	by 25. Youtd2 get_game_time() returns seconds.
	var can_proc: bool = last_proc_time + 5 < Utils.get_game_time() && autocast.get_manacost() > 0
	...
	last_proc_time = floori(Utils.get_game_time())
	buff.user_int = last_proc_time


func arcane_mana_replenish(target: Tower):
	var effect: int = Effect.create_colored("ReplenishHealthCasterOverhead.mdl", target.get_visual_x(), target.get_visual_y(), 80, 0.0, 1.0, Color8(100, 100, 255, 255))
