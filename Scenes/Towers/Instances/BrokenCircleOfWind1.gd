...
func _tower_init():
	...
	specials_modifier.add_modification(Unit.ModType.MOD_DMG_TO_AIR, 0.70, 0.02)
...
func _on_attack(event: Event):
	...
	if (target.get_size() == Unit.CreepSize.MASS || target.get_size() == Unit.CreepSize.NORMAL || target.get_size() == Unit.CreepSize.CHAMPION):
...
#   do damage
	if c.get_size() == Unit.CreepSize.CHAMPION:
		ratio = 1.25
