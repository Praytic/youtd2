...
#	NOTE: splash values are the same for all tiers
	...
	specials_modifier.add_modification(Unit.ModType.MOD_DMG_TO_MASS, _stats.value, _stats.value_add)
	specials_modifier.add_modification(Unit.ModType.MOD_DMG_TO_HUMANOID, _stats.value, _stats.value_add)
