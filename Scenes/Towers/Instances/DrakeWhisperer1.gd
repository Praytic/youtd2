...
func tower_init():
	...
	ely_drake_versatile_bt.set_buff_icon("@@0@@")
	ely_drake_versatile_bt.set_buff_tooltip("Versatile\nThis tower has increased dps.")
	...
	ely_drake_slow_bt.set_buff_icon("@@1@@")
...
func feeding(tower: Tower):
	...
	if feed_count < FEED_COUNT_MAX:
		...
		if corpse != null:
			...
			var effect: int = Effect.create_scaled("HumanBloodFootman.mdl", tower.get_visual_x() + 10, tower.get_visual_y(), tower.get_z() - 120, 0, 0.15)
...
func blue_drake_on_hit(p: Projectile, _target: Unit):
	...
	var effect: int = Effect.create_scaled("FrostNovaTarget.mdl", p.get_x(), p.get_y(), p.get_z(), 0, 0.2)
