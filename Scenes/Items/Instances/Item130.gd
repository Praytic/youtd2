...
func hippo_hit(p: Projectile, creep: Unit):
	...
	if creep.is_immune() == false:
		caster.do_spell_damage_aoe(p.x, p.y, 200, 1250 + caster.get_level() * 50, 1.0, caster.calc_spell_crit_no_bonus())
		var effect: int = Effect.create_scaled("AncientProtectorMissile", p.position.x, p.position.y, 0, 0, 0.8)
