...
func get_ability_info_list() -> Array[AbilityInfo]:
	...
	ability.description_short = "Burns hit creeps. Burned creep will receive extra damage from %s towers and explode after death, dealing AoE spell damage.\n" % [fire_string]
	ability.description_full = "Burns hit creeps. Burned creep will receive extra damage from %s towers. On every further hit of a %s tower, the target will receive more bonus damage then before. Towers from the same family as this tower will increase the bonus damage by %s, any other %s towers by %s. If the creep dies, it explodes and deals %s spell damage to nearby creeps in a range of 200.\n" % [fire_string, fire_string, bonus_damage, fire_string, bonus_damage_other, explode_damage] \
